...
You need two API keys. Get them before anything else.

#### Key 1 — OpenRouter (for all text AI models)
1. Go to **[openrouter.ai](https://openrouter.ai)** and create a free account
2. Click your profile → **Keys** → **Create Key**
3. Copy the key — it starts with `sk-or-v1-...`

#### Key 2 — Alibaba DashScope (for AI image generation)
1. Go to **[dashscope.aliyun.com](https://dashscope.aliyun.com)** and register
2. Go to **API-KEY管理** (API Key Management)
3. Click **创建API-KEY** (Create API Key)
4. Copy the key — it starts with `sk-...`

> Both services have a **free tier** — no credit card required to get started.
...
Wait for it to finish (takes ~1 minute). You should see packages installing.

> **If you see "pip3: command not found"**, install Python first at [python.org/downloads](https://www.python.org/downloads/) then try again.

---

### Step 4 — Add Your API Keys

Still in Terminal, run this to create your config file:
...
This opens the `.env` file in TextEdit. Replace the placeholder values with your real keys:

```
OPENROUTER_API_KEY=sk-or-v1-your-actual-key-here
IMAGE_API_KEY=sk-your-actual-dashscope-key-here
```
...
Once the website is open, you have two options:

**Option A — One click (recommended):**  
In the left sidebar, click **▶▶ Run Full Workflow**  
This runs all 5 tasks automatically in order. Takes 3–5 minutes.

**Option B — Step by step:**  
Click each tab in order and press the blue ▶ button in each one:
1. 📡 **Task 1** → Fetch News
2. 🎯 **Task 2** → Score Articles
3. 🏷️ **Task 3** → Classify Articles
4. 👤 **Task 4** → Load Style Guide
5. ✍️ **Task 5** → Generate Content

**Option C — Run without the browser (headless):**
```bash
python3 workflow.py
```
...
| Problem | Fix |
|---------|-----|
| `streamlit: command not found` | Run `pip3 install streamlit` first |
| `No API key` error in the app | Make sure you saved `.env` and **restarted** the app (Ctrl+C then `streamlit run app.py`) |
| `Image not yet generated` | Re-run Task 5 after adding your `IMAGE_API_KEY` and restarting the app |
